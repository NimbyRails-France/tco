...
bool initializeSdkClient(QString& error){
 ...
 if(!version||version(&info)!=NIMBY_OK||info.abi_version!=NIMBY_OBSERVATION_ABI_VERSION||info.major!=0||info.minor!=7){
  error="SDK incompatible : ce TCO nécessite NimbyRailsFranceSDK 0.7.x (pont 2). Réinstallez le paquet depuis NRF Hub.";return false;
 }
