...
class MapItem : public QQuickPaintedItem {
 ...
 Q_PROPERTY(double signalSize READ signalSize WRITE setSignalSize NOTIFY dataChanged)
public:
 explicit MapItem(QQuickItem* parent=nullptr);
 QVariantMap data() const{return data_;}
 void setData(const QVariantMap& value);
 void paint(QPainter* painter) override;
 double signalSize() const{return signalSize_;}
 void setSignalSize(double value);
